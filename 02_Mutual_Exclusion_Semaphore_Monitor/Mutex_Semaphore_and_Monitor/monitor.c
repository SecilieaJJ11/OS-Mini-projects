...
void *book(void *arg){
 ...
 pthread_mutex_lock(&m); while(seats[seat]) {printf("%s wait\n",names[i]); pthread_cond_wait(&c,&m);}
...
int main(){pthread_t t[4]; int id[]={0,1,2,3}; sem_init(&s,0,3); pthread_mutex_init(&m,0); pthread_cond_init(&c,0);
 for(int i=0;i<4;i++) pthread_create(&t[i],0,book,&id[i]); for(int i=0;i<4;i++) pthread_join(t[i],0); return 0;}
